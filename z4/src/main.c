...
void main(void) {
  ...
  while (1) {
    ...
    } else if (buttons->statesPending[0] &&
               buttons->held >= DEBOUNCING_PERIOD) {
      ctdnTimerSetSeconds(ctdnTimer,
        ctdnTimer->seconds - (
          buttons->held > 2 * T_MAX_SPD * HPS ? 10 + (
            ctdnTimer->seconds % 10
          ) : 1
        )
      );
    } else if (buttons->statesPending[1] &&
               buttons->held >= DEBOUNCING_PERIOD) {
      ctdnTimerSetSeconds(ctdnTimer,
        ctdnTimer->seconds + (
          buttons->held > 2 * T_MAX_SPD * HPS ? 10 + (
...
__interrupt void Timer_B1(void) {
  ...
  if (tbiv ==
      TBIV_TBCCR2) {  // Interwal inkrementacji / dekrementacji timera
    if (buttons->states[0]) {
    ...
    TBCCR2 += SET_PERIOD /
      (buttons->held > T_MAX_SPD * HPS ? 50 : (
      50 * buttons->held / (T_MAX_SPD * HPS) +
      1));
  ...
  if (tbiv == TBIV_TBCCR3) { // Interwal dlugosci nuty
    ...
    if (duration == 0) {
      ...
      if (pitch == 0) {
        ...
        TBCCTL4 &= ~OUTMOD_4;
        TBCCTL4 &= ~OUT;
      } else if (synth->index == 1 ||
                 synth->pitches[synth->index - 2] == 0) {
